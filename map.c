...
/*   Updated: 2016/01/26 00:08:46 by fnieto           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_buffer	map_to_vert_buff(t_list *map)
{
	...
	while (tmp[0])
	{
		...
		while (tmp[1])
		{
			buffer->buf[(buffer->w * (size_t)dim.y + (size_t)dim.x)] =
				str_vert(dim, (char*)(tmp[1]->content));
			tmp[1] = tmp[1]->next;
		}
		ft_lstdel(tmp1->content, &ft_lstfree_ptr);
		dim.y++;
		tmp[0] = tmp[0]->next;
	}
